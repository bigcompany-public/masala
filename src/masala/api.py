...
class AssetBlock:
    def __init__(
        ...
        load_callback: Callable[[AssetBlock, Path], None],
        ...
        self.loaded = False
        self.entry_points: dict[str, EntryPoint] = {ep.name: ep for ep in entry_points}
    ...
    def load(self, path: Path | None = None):
        ...
        self.load_callback(self, path)
        self.loaded = True


class EntryPoint:
    def __init__(self, name: str, label: str = "", str="", description: str = "") -> None:
        self.name = name
        self.label = label or name.replace("_", " ").replace("-", " ").title()
        self.description = description
